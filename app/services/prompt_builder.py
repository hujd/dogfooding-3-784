...
from ..models import Dimension, AnalyzeRequest


# ── 各维度专属角色定义（System Prompt）──────────────────

_SYSTEM_PROMPTS: dict[Dimension, str] = {
...
_TEMPLATES: dict[Dimension, str] = {
    Dimension.COMPREHENSION: """请分析以下代码的功能：
...
[简要描述代码使用的核心算法或逻辑]""",
    ...
    Dimension.BUG_DETECTION: """以下代码存在一个或多个 Bug，请找出所有问题：
...
[概括代码的整体质量和风险等级]""",
    ...
    Dimension.COMPLEXITY: """请分析以下代码的时间复杂度和空间复杂度：
...
[如果存在更优的算法，简要说明]""",
    ...
    Dimension.REFACTORING: """以下代码功能正确但存在设计问题，请给出重构建议：
...
| 可测试性 | ... | ... |""",
    ...
    Dimension.SECURITY: """请对以下代码进行安全审计：
...
[整体安全评级和建议]""",
    ...
    Dimension.EXECUTION_TRACE: """给定以下代码和输入，请推演执行过程并给出最终输出：
...
[列出执行过程中的关键分支和条件判断]""",
    ...
    Dimension.TRANSLATION: """请将以下 {language} 代码翻译为 {target_language}：
...
def build_prompt(req: AnalyzeRequest) -> tuple[str, str]:
    """根据请求构建 system + user 提示词"""
    system_prompt = _SYSTEM_PROMPTS[req.dimension]
    template = _TEMPLATES[req.dimension]
